<?php
	...
	function QcodoHandleException_pg(Exception $__exc_objException) {
		if (class_exists('QApplicationBase'))
			QApplicationBase::$ErrorFlag = true;
...
//		}
		...
		print $__exc_strMessage;	// just print out exception message without tracing
		// Call to display the Error Page (as defined in configuration.inc.php)
...
return;	// wpg - turn off this error reporting
	...
	function QcodoHandleError_pg($__exc_errno, $__exc_errstr, $__exc_errfile, $__exc_errline) {
		// If a command is called with "@", then we should return
		if (error_reporting() == 0)
			return;
		...
		$__exc_strMessage = $__exc_strObjectType;
		exit;
	}
