<?php
	...
	class QMySqlDatabase extends QMySqliDatabase {
		...
		public function Connect() {
			...
			$this->objDb = mysql_connect($strServer, $strUsername, $strPassword, true, MYSQL_CLIENT_SSL);
			if (!$this->objDb)
				throw new QMySqliDatabaseException("Unable to connect to Database Server: $strServer", -1, null);
			if (mysql_errno($this->objDb))
				throw new QMySqliDatabaseException(mysqli_error($this->objDb), mysql_errno($this->objDb), null);
		...
		public function Query($strQuery) {
			...
			if (mysql_errno($this->objDb))
				throw new QMySqliDatabaseException(mysqli_error($this->objDb), mysql_errno($this->objDb), $strQuery);
		...
		public function NonQuery($strNonQuery) {
			...
			if (mysql_errno($this->objDb))
				throw new QMySqliDatabaseException(mysqli_error($this->objDb), mysql_errno($this->objDb), $strNonQuery);
		}
		...
		public function GetFieldsForTable($strTableName) {
			...
			$objResult = new QMySqlDatabaseResult(mysql_query("SHOW FULL COLUMNS FROM ".$strTableName." IN ".$this->Database), $this);
	...
	class QMySqlDatabaseField extends QMySqliDatabaseField {
		public function __construct($mixFieldData, $objDb = null) {
			...
			$this->strComment = $mixFieldData->Comment;
