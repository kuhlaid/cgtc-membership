<?php
	class QMySqliDatabase extends QDatabaseBase {
		...
		public function Connect() {
			...
			$this->objMySqli = new MySqli($this->Server, $this->Username, $this->Password, $this->Database, $this->Port, MYSQL_CLIENT_SSL);
		...
		// wpg - modified GetFieldsForTable function to capture Comment for each field
		...
		// wpg
//		public function GetFullColumnsForTable($strTableName) {
//			// Connect if Applicable
//			if (!$this->blnConnectedFlag) $this->Connect();
//			
//			// wpg - changed this query to pull ALL columns from the table so we can see comments
//			// $objResult = new QMySqlDatabaseResult(mysql_list_fields($this->Database, $strTableName, $this->objDb), $this);
//			$objResult = $this->Query("SHOW FULL COLUMNS FROM ".$strTableName." IN ".$this->Database);
//			return $objResult->FetchFields();
//		}
	...
	class QMySqliDatabaseField extends QDatabaseFieldBase {
		public function __construct($mixFieldData, $objDb = null, $strComment = null) {
		...
		protected function SetFieldType($intMySqlFieldType) {
			switch ($intMySqlFieldType) {
				...
				case MYSQLI_TYPE_GEOMETRY:
					...
					break;
				default:
					throw new Exception("Unable to determine MySqli Database Field Type: " . $intMySqlFieldType . " but might be ". MYSQLI_TYPE_JSON);
					break;
			}
		}
	}
