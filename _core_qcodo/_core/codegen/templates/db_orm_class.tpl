...
<% } %>
		...
		protected static function BuildQueryStatement(&$objQueryBuilder, QQCondition $objConditions, $objOptionalClauses, $mixParameterArray, $blnCountOnly,$selectionArray = null) {
			...
			<%= $objTable->ClassName %>::GetSelectFields($objQueryBuilder,null,$selectionArray);
			$objQueryBuilder->AddFromItem('<%= $strDbSchema %><%= $strEscapeIdentifierBegin %><%= $objTable->Name %><%= $strEscapeIdentifierEnd %> AS <%= $strEscapeIdentifierBegin %><%= $objTable->Name %><%= $strEscapeIdentifierEnd %>');
		...
		public static function QuerySingle(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null,$selectionArray = null) {
			...
			try {
				$strQuery = <%= $objTable->ClassName %>::BuildQueryStatement($objQueryBuilder, $objConditions, $objOptionalClauses, $mixParameterArray, false,$selectionArray);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
			}
		...
		public static function QueryCount(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null,$selectionArray = null) {
			// Get the Query Statement
			try {
				$strQuery = <%= $objTable->ClassName %>::BuildQueryStatement($objQueryBuilder, $objConditions, $objOptionalClauses, $mixParameterArray, true,$selectionArray);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
			}
	...
	<% } %>
			...
			elseif($selectionArray==array(1)) {
				...
				foreach($selectionArray AS $field){
					$objBuilder->AddSelectItem($strTableName . '.<%= $strEscapeIdentifierBegin %>'.$field.'<%= $strEscapeIdentifierEnd %> AS ' . $strAliasPrefix . $field.'<%= $strEscapeIdentifierEnd %>');
				}
			}
		}
		...
		*/
		public static function DeleteAll() {
			...
			$objDatabase->NonQuery('
				DELETE FROM
					<%= $strDbSchema %><%= $strEscapeIdentifierBegin %><%= $objTable->Name %><%= $strEscapeIdentifierEnd %>');
		}
...
<% if ($this->blnManualQuerySupport) { %>
		...
		public static function ExpandQuery($strParentAlias, $strAlias, $objExpansionMap, QQueryExpansion $objQueryExpansion) {
			if ($strAlias) {
				$objQueryExpansion->AddFromItem(sprintf('LEFT JOIN <%= $strDbSchema %><%= $strEscapeIdentifierBegin %><%= $objTable->Name %><%= $strEscapeIdentifierEnd %> AS <%= $strEscapeIdentifierBegin %>%s__%s<%= $strEscapeIdentifierEnd %> ON <%= $strEscapeIdentifierBegin %>%s<%= $strEscapeIdentifierEnd %>.<%= $strEscapeIdentifierBegin %>%s<%= $strEscapeIdentifierEnd %> = <%= $strEscapeIdentifierBegin %>%s__%s<%= $strEscapeIdentifierEnd %>.<%= $strEscapeIdentifierBegin %><%= $objTable->PrimaryKeyColumnArray[0]->Name %><%= $strEscapeIdentifierEnd %>', $strParentAlias, $strAlias, $strParentAlias, $strAlias, $strParentAlias, $strAlias));

<% foreach ($objTable->ColumnArray as $objColumn) { %>
				$objQueryExpansion->AddSelectItem(sprintf('<%= $strDbSchema %><%= $strEscapeIdentifierBegin %>%s__%s<%= $strEscapeIdentifierEnd %>.<%= $strEscapeIdentifierBegin %><%= $objColumn->Name %><%= $strEscapeIdentifierEnd %> AS <%= $strEscapeIdentifierBegin %>%s__%s__<%= $objColumn->Name %><%= $strEscapeIdentifierEnd %>', $strParentAlias, $strAlias, $strParentAlias, $strAlias));
<% } %>
...
<% foreach ($objTable->ColumnArray as $objColumn) { %>
	<% if ($objColumn->VariableType == QType::DateTime) { %>
			if ($objObject-><%= $objColumn->VariableName %>)
				$objObject-><%= $objColumn->VariableName %> = $objObject-><%= $objColumn->VariableName %>->toString(QDateTime::FormatSoap);
	<% } %><% if ($objColumn->Reference && (!$objColumn->Reference->IsType)) { %>
...
<% foreach ($objTable->ManyToManyReferenceArray as $objReference) { %>
	class QQNode<%= $objTable->ClassName %><%= $objReference->ObjectDescription %> extends QQAssociationNode {
		...
		protected $strDbSchema = '<%= $strDbSchema %>';	// wpg - added so we would have the database schema
