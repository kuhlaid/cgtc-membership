		...
		protected function <%= $strControlId %>_Create() {
			...
			$this-><%= $strControlId %>->Name = QApplication::Translate('<%= $objColumn->Comment; %> [<%= $objColumn->Name; %>]'); 	//QApplication::Translate('<%= QConvertNotation::WordsFromCamelCase($objColumn->PropertyName) %>');
