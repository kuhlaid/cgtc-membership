<?php
	require(__QCODO__ .'/codegen/QConvertNotation.class.php');
	...
	abstract class QCodeGenBase extends QBaseClass {
		...
		public static function GenerateAggregate() {
			...
			$strToReturn = array();
			array_push($strToReturn, QDatabaseCodeGen::GenerateAggregateHelper($objDbOrmCodeGen));
//			array_push($strToReturn, QRestServiceCodeGen::GenerateAggregateHelper($objRestServiceCodeGen));
		 ...
		 * all the *.tpl templates from QCodeGen::TemplatesPath and QCodeGen::TemplatesPathCustom containing
		 * the prefix, and call GenerateFile() on each one.  If there are any template files that reside
		...
		public function GenerateFiles($strTemplatePrefix, $mixArgumentArray) {
			...
			$strTemplatePath = sprintf('%s%s', __QCODO_CORE__, QCodeGen::TemplatesPath);
			...
			// Next, go through the set of custom templates
			...
			$blnSuccess = true;
			foreach ($strTemplateFileArray as $strTemplateFilePath) {
				if (!$this->GenerateFile($strTemplateFilePath, $mixArgumentArray))
					$blnSuccess = false;
			}
		...
		/**
		 * Given an absolutely defined TemplateFilePath, go ahead and call EvaluateTemplate,
		 * using the passed in $mixArgumentArray, to generate the file against the template.
		 * If $blnSave is true, then save it using the information in the template's <template>
		 * tag.
		 *
		 * If saving, returns true/false of its success.  If NOT saving, then it returns the contents
		 * of the evaluated template.
		 *
		 * @param string $strTemplateFilePath
		 ...
		 */
		public function GenerateFile($strTemplateFilePath, $mixArgumentArray, $blnSave = true) {		
			...
			$strTemplate = $this->EvaluateTemplate($strTemplate, $mixArgumentArray);
		...
		protected function EvaluateSubTemplate($strSubTemplateFilename, $mixArgumentArray) {
			if (QCodeGen::DebugMode) _p("Evaluating $strSubTemplateFilename<br/>", false);

			// Try the Custom SubTemplate Path
			$strFilename = sprintf('%s%s%s', __QCODO__, QCodeGen::SubTemplatesPathCustom, $strSubTemplateFilename);
			if (file_exists($strFilename))
				return $this->EvaluateTemplate(file_get_contents($strFilename), $mixArgumentArray);

			// Try the Standard SubTemplate Path
			$strFilename = sprintf('%s%s%s', __QCODO_CORE__, QCodeGen::SubTemplatesPath, $strSubTemplateFilename);
			if (file_exists($strFilename))
				return $this->EvaluateTemplate(file_get_contents($strFilename), $mixArgumentArray);

			// SubTemplate Does Not Exist
			throw new QCallerException('CodeGen SubTemplate Does Not Exist: ' . $strSubTemplateFilename);
		}
		...
		protected function EvaluateTemplate($strTemplate, $mixArgumentArray) {
			...
			// wpg - we want to get the database schema to add to the generated code
			$strDbSchema = QApplication::$Database[$this->intDatabaseIndex]->DbSchema;
			//error_log("Schema:".$strDbSchema);
			...
			while ($intPosition !== false) {
				...
				if (substr($strStatement, 0, 1) == '=') {
					...
					$strTemplateFile = sprintf('%s%s%s.tpl', __QCODO__, QCodeGen::SubTemplatesPathCustom, $strTemplateFileName);
					if (!file_exists($strTemplateFile)) {
						$strTemplateFile = sprintf('%s%s%s.tpl', __QCODO_CORE__, QCodeGen::SubTemplatesPath, $strTemplateFileName);
						if (!file_exists($strTemplateFile))
		...
		protected function VariableTypeFromDbType($strDbType) {
			switch ($strDbType) {
				case QDatabaseFieldType::BigInt:
					return QType::String;
