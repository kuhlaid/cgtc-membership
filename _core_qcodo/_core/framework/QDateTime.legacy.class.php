<?php
	...
	class QDateTime extends QBaseClass {
		...
		/**
		 * wpg - added October 2008
		 * MySQL formatted date and time
		 */
		const FormatMySQL = 'YYYY-MM-DD hh:mm:ss';

		/**
		 * wpg - added January 2008
		 * Constant for __toString to display Date with slashes
		 * 12/09/2003
		 */
		const FormatSlashes = 'MM/DD/YYYY';
		...
		/**
		 * wpg - Constant for __toString to display Time as without seconds
 		 * 03:35:15 PM
		 */
		const FormatDisplayTimeNoSeconds = 'hh:mm zz';
		/**
		...
		public function __construct($mixValue = null) {
			// wpg - added this bit of code to handle QDateTimeTextBox text in MMM DD YYYY format
			if (!($mixValue instanceof QDateTime)) {
				...
				$calFormatArray = '';
				$mixValueArray = explode(" ", $mixValue);
				// find out if we are displaying in default MMM DD YYYY format
				foreach ($monthArray as $key => $value){
					// when we find out what month we are on, create the date string
					if (strpos(strtolower($mixValueArray[0]), strtolower($value))===0) {

						$calFormatArray = array(0=>$mixValueArray[2],1=>$key,2=>$mixValueArray[1]);
						break;
					}
				}
			}
			...
			// wpg - added this function to handle the QCalendar function default format
			elseif ($calFormatArray) {
				$this->intMonth = $calFormatArray[1];
				$this->intDay = $calFormatArray[2];
				$this->intYear = $calFormatArray[0];
			}
		...
		// wpg - created this function because there seems to be a problem with the Difference function
		// this function just returns the number of days difference and not another object
		public function intDifference(QDateTime $dttDateTime) {
			$intDifference = $this->Day - $dttDateTime->Day;
			return $intDifference;
		}

		// wpg - added Feb. 2010
		public function SubtractDays($intDays){
			$this->Day -= $intDays;
			return $this;
		}
		...
		/**
		 * wpg - added to act like the new QDateTime class
		 *
		 * @param unknown_type $intDays
		 */
		...
		public function toString($strFormat = null) {
			if (is_null($strFormat))
				$strFormat = QDateTime::$DefaultFormat;
			$strArray = preg_split('/([^D^M^Y^h^m^s^z^t^w])+/', $strFormat);
			...
			*/
			...
			// wpg - updated to use the latest date time parser (otherwise a to string on YYYYMMDD will not be parsed)
			//preg_match_all('/(?(?=D)([D]+)|(?(?=M)([M]+)|(?(?=Y)([Y]+)|(?(?=h)([h]+)|(?(?=m)([m]+)|(?(?=s)([s]+)|(?(?=z)([z]+)|(?(?=t)([t]+)|(?(?=w)([w]+)|)))))))))/', $strFormat, $strArray);
			//$strArray = $strArray[0];
			$strToReturn = '';
			...
			for ($intIndex = 0; $intIndex < count($strArray); $intIndex++) {
				...
				if ($strToken) {
					...
					switch ($strArray[$intIndex]) {
						...
						// wpg - added to return the day of the week (integer)
						case 'w':
							$strToReturn .= date('w', $intTimestamp);
							break;
		...
		public function GetSoapDateTimeArray($dttArray) {
			...
			foreach ($dttArray as $dttItem)
				array_push($strArrayToReturn, $dttItem->toString(QDateTime::FormatSoap));
