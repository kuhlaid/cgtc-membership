<?php
	...
	abstract class QEmailServer extends QBaseClass {
		...
		public static function Send(QEmailMessage $objMessage) {
			$objResource = null;
			error_log("sending email with Send function".QEmailServer::$SmtpServer.QEmailServer::$SmtpPort);
			if (QEmailServer::$TestMode) {
				...
				// wpg - trying to see if curl will connect using smtp protocol (no luck, it isn't supported)
//				if (!curl_init()) { print "Error: Authentication can not be run with the current setup."; exit; }
//				$ch = curl_init();
//				curl_setopt($ch, CURLOPT_URL, QEmailServer::$SmtpServer);
//        		curl_setopt($ch, CURLOPT_PORT, QEmailServer::$SmtpPort);
...
// wpg - commented out because it seemed to be causing problems with the html output when this was combined with the html body
//				fwrite($objResource, sprintf("--%s\r\n", $strAltBoundary));
...
//				fwrite($objResource, "\r\n\r\n");
				...
				// wpg - adding this encoding instead of the printable so the html body would come out correctly
				fwrite($objResource, sprintf("Content-Transfer-Encoding: 7bit\r\n\r\n"));

				// wpg - commented out because it will screw up the html body with links
...
//					}
			...
			throw new QEmailException(sprintf('Throwing because I want'));
			error_log("email with Send function should have been sent");
			// Send: QUIT
