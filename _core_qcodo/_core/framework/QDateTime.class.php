<?php
	...
	class QDateTime extends DateTime {
		...
		const FormatDisplayDate = 'MMMM DD, YYYY';
		 ...
		 * @return QDateTime
		 */
		public static function FromTimestamp($intTimestamp = null, ? DateTimeZone $objTimeZone = null) {
...
//			$strArray = preg_split('/([^D^M^Y^h^m^s^z^t])+/', $strFormat);
			...
			for ($intIndex = 0; $intIndex < count($strArray); $intIndex++) {
				...
				if ($strToken) {
					...
					switch ($strArray[$intIndex]) {
						...
						case 'w':
							$strToReturn .= parent::format('w');	//date('w', $intTimestamp);
							break;
		...
		public function setTime(? int $intHour = null, ? int $intMinute = null, ? int $intSecond=NULL, ? int $microseconds = 0): DateTime {
...
//	$dtt2->Month++;
	...
	function printDate($dtt) {
		...
		print ('<br/>');
		print ('Date: ' . $dtt->toString(QDateTime::FormatDisplayDateTimeFull));
		print ('<br/>');
