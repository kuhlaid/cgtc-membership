<?php
	...
	class QCheckBoxList extends QListControl {
		...
		protected function GetControlHtml() {
			...
			// Generate Table HTML
//			$strToReturn = sprintf('<table id="%s" %s%s %s%s%s%s%s>',
//				$this->strControlId,
//				$strCellPadding,
//				$strCellSpacing,
//				$strAccessKey,
...
//				$strCustomAttributes);
			...
			// wpg - switched to div from table
				...
				for ($intRowIndex = 0; $intRowIndex < $intRowCount; $intRowIndex++) {
					...
					for ($intColIndex = 0; $intColIndex < $intColCount; $intColIndex++) {
						...
						if ($this->strTextAlign == QTextAlign::Left) {
							$strToReturn .= sprintf('<div '.$strCssClass.'>%s<label for="%s[%s]">%s</label><input id="%s[%s]" name="%s[%s]" type="checkbox" %s%s%s%s />%s</div>',
								$strDisabledStart,
								$this->strControlId,
								$intIndex,
							...
							$strToReturn .= sprintf('<div '.$strCssClass.'>%s<input id="%s[%s]" name="%s[%s]" type="checkbox" %s%s%s%s /><label for="%s[%s]">%s</label>%s</div>',
								$strDisabledStart,
								$this->strControlId,
								$intIndex,
								$this->strControlId,
