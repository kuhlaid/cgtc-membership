<?php
	...
	class QDataGridColumn extends QBaseClass {
		...
		// MISC
		protected $strName, $strFooter;
		protected $strHtml;
		protected $blnHtmlEntities = true;

		// wpg - added a footer entry to the construct so we can add footer information to the datagrid
		public function __construct($strName, $strHtml = null, $objOverrideParameters = null, $strFooter = null) {
			...
			if (count($objOverrideArray) > 2)
				try {
					...
					unset($objOverrideArray[3]);	// wpg - we do not want to include the footer attribute as an override parameter
		...
		public function __set($strName, $mixValue) {
			switch ($strName) {
				...
				case "Footer":
					try {
						$this->strFooter = QType::Cast($mixValue, QType::String);
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
						throw $objExc;
