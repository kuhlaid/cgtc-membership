<?php
	...
	abstract class QControlBase extends QBaseClass {
		...
		// wpg - ADDED TO CONTROL HOW INPUT FIELDS ARE RENDERED IN THE CASE WE WANT TO SWITCH TO PRINT ONLY MODE
		protected $blnLabelMode = false;
		
		// wpg - added to control how input fields are rendered in case we want to just view a blank form for printing
		protected $blnViewOnlyMode = false;
...
//		abstract public function GetJavaScriptAction();
		...
		public function __set($strName, $mixValue) {
			...
			switch ($strName) {
				...
				// wpg - added to handle changing a text box to a label for faster view only forms
				case "LabelMode":
					try {
						$this->blnLabelMode = QType::Cast($mixValue, QType::Boolean);
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
						throw $objExc;
					}
				// wpg added for view only of forms for printing
				case "ViewOnlyMode":
					try {
						$this->blnViewOnlyMode = QType::Cast($mixValue, QType::Boolean);
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
						throw $objExc;
