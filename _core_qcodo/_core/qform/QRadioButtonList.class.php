...
//				$strCustomAttributes);
			...
			// wpg - modified code to not use tables
			$strToReturn = sprintf('<div id="%s" class="rbWrapper" %s%s%s%s>',
				$this->strControlId,
				$strAccessKey,
				$strToolTip,
				$strStyle,
				...
				for ($intRowIndex = 0; $intRowIndex < $intRowCount; $intRowIndex++) {
					...
					for ($intColIndex = 0; $intColIndex < $intColCount; $intColIndex++) {
						...
						if ($this->blnEnabled) {
							...
							$strDisabledStart = '<span class="disabled">';
							$strDisabledEnd = '</span>';
							$strDisabled = 'disabled="disabled" ';
						}

						if ($this->strTextAlign == QTextAlign::Left) {
							$strToReturn .= sprintf('<div '.$strCssClass.'>%s<label for="%s_%s">%s</label><input title="'.$this->Name.'" id="%s_%s" name="%s" value="%s" type="radio" %s%s%s%s />%s</div>',
								$strDisabledStart,
								$this->strControlId,
								$intIndex,
							...
							$strToReturn .= sprintf('<div '.$strCssClass.'>%s<input title="'.$this->Name.'" id="%s_%s" name="%s" value="%s" type="radio" %s%s%s%s /><label for="%s_%s">%s</label>%s</div>',
								$strDisabledStart,
								$this->strControlId,
								$intIndex,
								$this->strControlId,
