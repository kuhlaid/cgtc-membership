<?php
	...
	class QCalendar extends QControl {
		...
		protected $dblMinimumDate = null;	// needs to be a timestamp for a day
		protected $dblMaximumDate = null;	// needs to be a timestamp for a day
		...
		protected function GetControlHtml() {
			...
			if ($this->intTimestamp) {
				...
				$strDate = $this->DateTime->toString('MMMM DD, YYYY');	// wpg changed to display full date
			} else {
				$intTimestamp = "";
				$strDate = "";
			}
			...
			switch ($this->strCalendarType) {
				...
				case QCalendarType::DateTimeSeconds:
					...
					$strToReturn .= sprintf('<input type="text" onFocus="this.blur()" onClick="__calendar(%s, %s, %s, %s)" name="%s" id="%s" value="%s" %s%s>',
						"'" . $this->Form->FormId . "'",
						"'" . $this->strControlId . "'",
						"'" .$this->dblMinimumDate. "'",
						"'" .$this->dblMaximumDate. "'",
					...
					if ($this->blnEnabled) {
						$strToReturn .= sprintf(' <a href="javascript:__calendar(%s, %s, %s, %s)"><img src="%s/calendar.png" border="0"></a>',
							"'" . $this->Form->FormId . "'",
							"'" . $this->strControlId . "'",
							"'" .$this->dblMinimumDate. "'",
							"'" .$this->dblMaximumDate. "'",
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case "MinimumDate": return $this->dblMinimumDate;
				case "MaximumDate": return $this->dblMaximumDate;
				default:
					try {
		...
		public function __set($strName, $mixValue) {
			...
			switch ($strName) {
				...
				case "MinimumDate": $this->dblMinimumDate = QType::Cast($mixValue, QType::String); break;
				case "MaximumDate": $this->dblMaximumDate = QType::Cast($mixValue, QType::String); break;
				default:
					try {
