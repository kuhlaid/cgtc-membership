	...
	abstract class QTextBoxBase extends QControl {
		...
		protected function GetControlHtml() {
			...
			switch ($this->strTextMode) {
				case QTextMode::MultiLine:
					$strToReturn = sprintf('<textarea title="%s" name="%s" id="%s" %s%s>%s</textarea>',
						$this->Name,
						$this->strControlId,
						$this->strControlId,
						$this->GetAttributes(),
				...
				default:
					$strToReturn = sprintf('<input type="text" title="%s" name="%s" id="%s" value="%s" %s%s />',
						$this->Name,
						$this->strControlId,
						$this->strControlId,
						QApplication::HtmlEntities($this->strText),
		...
		public function __get($strName) {
			switch ($strName) {
				...
				// wpg - reverting back to nornal since the other function was causing Spanish characters to become corrupt
				case "Text": return $this->strText;	//iconv("UTF-8", "ISO-8859-1//IGNORE", $this->strText); // added to ignore crazy characters but it distorts Spanish translations
		...
		public function __set($strName, $mixValue) {
			...
			// wpg - added to ignore crazy characters
			//$mixValue = iconv("UTF-8", "ISO-8859-1//IGNORE", $mixValue);
