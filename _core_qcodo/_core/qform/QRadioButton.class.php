		...
		protected function GetControlHtml() {
			...
			if (strlen($this->strText ?? '')) {
				...
				if ($this->strTextAlign == QTextAlign::Left) {
					...
					$strToReturn = sprintf('<span %s%s%s%s%s><input type="radio" title="'.$this->Name.'" id="%s" name="%s" value="%s" %s%s%s%s%s /><label for="%s">%s</label></span>',
						$strCssClass,
						$strToolTip,
						$strStyle,
						$strCustomAttributes,
				...
				$strToReturn = sprintf('<input type="radio" title="'.$this->Name.'" id="%s" name="%s" value="%s" %s%s%s%s%s%s%s%s%s />',
					$this->strControlId,
					$strGroupName,
					$this->strControlId,
					$strCssClass,
