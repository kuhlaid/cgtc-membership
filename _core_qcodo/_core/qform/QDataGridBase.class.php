...
//			$this->objColumnArray[count($this->objColumnArray)] = $objColumn;
		...
		protected function GetPaginatorRowHtml($objPaginator) {
			$strToReturn = sprintf('<tr><td colspan="%s" class="resultsPane"><table class="results"><tr><td class="resultsPagieC1">', count($this->objColumnArray));
			...
			if ($this->TotalItemCount > 0) {
				...
				// wpg - added this variable check because before it was always showing the plural noun
				$tempNoun = $this->strNoun;
				if ($this->TotalItemCount > 1) $tempNoun = $this->strNounPlural;

				$itemsPerPage = '';
				// show or hide items per page
				if ($this->ItemsPerPage != '') $itemsPerPage = __PAGINATION_OPTIONS__;
			...
			$strToReturn .= '</td><td class="resultsPagieC2">';
		...
		protected function GetHeaderSortedHtml(QDataGridColumn $objColumn) {
			$strToReturn = sprintf('<span class="dgSortColumnHdr">%s</span>', $objColumn->Name);
		...
		// wpg - added basic html attribute
		protected function GetHeaderRowHtml($basic = null, $removeHtml = 0) {
			...
			if ($basic)
				...
				$strToReturn = sprintf('<tr %s class="headerRow">', $objHeaderStyle->GetAttributes());
		...
		protected function GetFooterRowHtml($removeHtml = 0) {
			...
			$strToReturn = sprintf('<tr %s>', $objFooterStyle->GetAttributes());
			$intColumnIndex = 0;
			if ($this->objColumnArray) foreach ($this->objColumnArray as $objColumn) {
		...
		// wpg - get read only view of data for export
		protected function GetControlHtmlReadOnlyPHPExcel() {
			$this->DataBind();
			...
			if ($this->objDataSource)
				foreach ($this->objDataSource as $objObject)
					$strToReturn .= $this->GetDataGridRowHtml($objObject,1,1);
