<?php
	...
	class QFloatTextBox extends QTextBox {
		...
		public function __construct($objParentObject, $strControlId = null) {
			...
			$this->strLabelForInvalid = QApplication::Translate('Only numbers and decimal points allowed in this field');
			$this->strLabelForLess = QApplication::Translate('Value must be less than or equal to %s');
			$this->strLabelForGreater = QApplication::Translate('Value must be greater than or equal to %s');
		}

		// wpg - added  $parentOveride argument and handler so we could validate a single field and not the entire form
		// An example would be something like $this->txtQ1->Validate(true); // that is called on a change event for the text field for instance
		public function Validate($parentOveride = false) {
			if (parent::Validate() || $parentOveride) {
				if ($this->strText != '') {
					try {
